...
NewsURL is a Streamlit app that lets you paste **news article URLs**, build a **FAISS vector index**, and then ask questions with **retrieved evidence + (optional) Gemini answers**.

The app supports:
- **Gemini embeddings + Gemini answers** (requires `GOOGLE_API_KEY` / `GEMINI_API_KEY`)
- **Local embeddings** via `sentence-transformers` (works without an API key; heavier dependency footprint)
...
- A Gemini key if you want Gemini answering:
